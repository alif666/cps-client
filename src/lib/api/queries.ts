...
export const createPermission = (body: { code: string; name: string; description?: string }) => apiFetch<Permission>('/access/permissions', { method: 'POST', body: JSON.stringify(body) });
export const assignRolePermissions = (roleId: string, permissionIds: string[]) => apiFetch<Role>(`/access/roles/${roleId}/permissions`, { method: 'PATCH', body: JSON.stringify({ permissionIds }) });
...
export const createBusinessUnit = (body: { companyId: string; code: string; name: string }) => apiFetch<AccessEntity>('/access/business-units', { method: 'POST', body: JSON.stringify(body) });
...
export const createDepartment = (body: { businessUnitId: string; code: string; name: string }) => apiFetch<AccessEntity>('/access/departments', { method: 'POST', body: JSON.stringify(body) });
export const listStores = (companyId: string) => apiFetch<AccessEntity[]>(`/access/stores?companyId=${encodeURIComponent(companyId)}`);
export const createStore = (body: { companyId: string; businessUnitId?: string; code: string; name: string; location?: string }) => apiFetch<AccessEntity>('/access/stores', { method: 'POST', body: JSON.stringify(body) });
